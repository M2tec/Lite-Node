...
if [[ $NETWORK == "traefik" ]]; then
    docker compose -f docker-compose-traefik.yml --env-file .env.preprod up -d
else
    docker compose --env-file .env.${NETWORK} -p koios-${NETWORK} -f "${KLITE_HOME}"/docker-compose.yml up -d
fi